...
def test_obtener_equipos_pagina_1():
    pagina = 1
    test_1 = test.get(f"/equipos/?pagina={pagina}&cantidad_pokemons=10")
    assert test_1.status_code == 200
    assert len(test_1.json()) == 10

def test_obtener_equipos_pagina_sin_equipos():
    pagina = 4
    test_2 = test.get(f"/equipos/?pagina={pagina}&cantidad_pokemons=10")
...
def test_pagina_invalida():
    pagina = 0
    test_3 = test.get(f"/equipos/?pagina={pagina}&cantidad_pokemons=10")
...
def test_cantidad_olvidada():
    cantidad = 0
    test_4 = test.get(f"/equipos/?pagina=1&cantidad_pokemons={cantidad}")
