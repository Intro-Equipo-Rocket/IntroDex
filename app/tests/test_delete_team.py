...
def setup_db():
    ...
    with Session(engine) as session:
        session.add_all([
            ...
            Estadisticas(id=1, member_id=1, vida=31, ataque=31, defensa=31, ataque_especial=20, defensa_especial=30, velocidad=50),
            Estadisticas(id=2, member_id=2, vida=20, ataque=25, defensa=15, ataque_especial=40, defensa_especial=30, velocidad=50)
        ])
        session.commit()
    yield
...
def test_eliminar_equipo_existente(client, setup_db):
    ...
    assert response.json() == {'detail': f'El equipo con id {equipo_id} y sus integrantes han sido eliminados'}
    ...
    with Session(engine) as session:
        ...
        evs = session.exec(select(Estadisticas).where(Estadisticas.member_id.in_([1, 2]))).all()
        assert len(evs) == 0
...
def test_eliminar_equipo_inexistente(client, setup_db):
    ...
    assert response.json() == {'detail': f'El equipo con id {equipo_id} no ha sido encontrado'}
