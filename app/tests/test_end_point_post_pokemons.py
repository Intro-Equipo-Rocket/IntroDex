...
def test_crear_pokemon_no_existente():
    ...
    client.delete(f"/pokemons/delete/{model_pokemon_no_existente['nuevo_pokemon']['id']}")
...
def test_crear_pokemon_no_existente_con_movimiento_inexistente():
    ...
    response = client.post("/pokemons", json=model_pokemon_no_existente)
    assert response.status_code == 400
    assert response.json() == {"detail": "El movimiento no existe (o no se ha asignado correctamente)"}
...
def test_crear_pokemon_no_existente_con_movimientos_repetidos():
    model_pokemon_no_existente["nuevos_movimientos"].append(model_pokemon_no_existente["nuevos_movimientos"][0])
    response = client.post("/pokemons", json=model_pokemon_no_existente)
    assert response.status_code == 400
...
def test_crear_pokemon_no_existente_sin_tipos():
    ...
    model_pokemon_no_existente["nuevos_tipos"] = [
        {
            "type_id": 5
        },
        {
...
def test_crear_pokemon_no_existente_con_mas_de_dos_tipos():
    model_pokemon_no_existente["nuevos_tipos"].append(model_pokemon_no_existente["nuevos_tipos"][0])
    response = client.post("/pokemons", json=model_pokemon_no_existente)
    assert response.status_code == 400
...
def test_crear_pokemon_no_existente_con_tipo_inexistente():
    ...
    response = client.post("/pokemons", json=model_pokemon_no_existente)
    assert response.status_code == 400
    assert response.json() == {"detail": "El tipo no existe (o no se ha asignado correctamente)"}
...
def test_crear_pokemon_no_existente_con_habilidad_inexistente():
    ...
    response = client.post("/pokemons", json=model_pokemon_no_existente)
    assert response.status_code == 400
    assert response.json() == {"detail": "La habilidad no existe (o no se ha asignado correctamente)"}
...
def test_crear_pokemon_no_existente_con_habilidades_repetidas():
    model_pokemon_no_existente["nuevas_habilidades"].append(model_pokemon_no_existente["nuevas_habilidades"][0])
    response = client.post("/pokemons", json=model_pokemon_no_existente)
    assert response.status_code == 400
...
def test_crear_pokemon_no_existente_sin_grupo_huevo():
    ...
    response = client.post("/pokemons", json=model_pokemon_no_existente)
    assert response.status_code == 400
    assert response.json() == {"detail": "El Pokemon debe tener al menos un grupo huevo"}
    model_pokemon_no_existente["nuevas_especies"] = [
        {
            "egg_group_id": 15
        }
...
def test_crear_pokemon_no_existente_con_grupo_huevo_inexistente():
    ...
    response = client.post("/pokemons", json=model_pokemon_no_existente)
    assert response.status_code == 400
    assert response.json() == {"detail": "El grupo huevo no existe (o no se ha asignado correctamente)"}
...
def test_crear_pokemon_no_existente_con_grupos_huevo_repetidos():
    model_pokemon_no_existente["nuevas_especies"].append(model_pokemon_no_existente["nuevas_especies"][0])
    response = client.post("/pokemons", json=model_pokemon_no_existente)
    assert response.status_code == 400
...
def test_crear_pokemon_no_existente_con_stats_de_mas():
    model_pokemon_no_existente["nuevas_stats"].append(model_pokemon_no_existente["nuevas_stats"][0])
    response = client.post("/pokemons", json=model_pokemon_no_existente)
    assert response.status_code == 400
    assert response.json() == {"detail": "El Pokemon debe tener 6 stats"}
...
def test_crear_pokemon_no_existente_con_stat_inexistente():
    ...
    response = client.post("/pokemons", json=model_pokemon_no_existente)
    assert response.status_code == 400
    assert response.json() == {"detail": "El stat no existe (o no se ha asignado correctamente, asegúrate de que los ids de los stats sean del 1 al 6)"}
    model_pokemon_no_existente["nuevas_stats"][0]["stat_id"] = 1
